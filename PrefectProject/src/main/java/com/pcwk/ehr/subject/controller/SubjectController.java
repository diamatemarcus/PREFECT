...
public class SubjectController implements PcwkLogger {
	...
	public String doRetrieve(SubjectVO searchVO, @RequestParam(value = "searchDiv", required = false) String searchDiv,
		...
		String view = "subject/score_list";
		String email = "";
	...
	// 출석 리스트
	@GetMapping(value = "/moveToScoreReg.do")
	...
	public String moveToSubjectReg(SubjectVO inVO, Model model, HttpSession httpSession)
			throws SQLException, EmptyResultDataAccessException, ParseException {
		String view = "subject/subject_reg";
	...
	@GetMapping(value = "/moveToSubjectList.do")
	public String moveToSubjectList(SubjectVO inVO, Model model, HttpSession httpSession)
			throws SQLException, EmptyResultDataAccessException, ParseException {
		String view = "subject/subject_reg";
	...
	public String doSelectOne(@RequestParam("trainee") String trainee, @RequestParam("subjectCode") int subjectCode,
			@RequestParam("coursesCode") int coursesCode, Model model) throws SQLException {
	...
	// 등록
	@RequestMapping(value = "/doSaveSubject.do", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	@ResponseBody // HTTP 요청 부분의 body부분이 그대로 브라우저에 전달된다.
	public String doSaveSubject(CodeVO inVO) throws SQLException {
		String jsonString = "";
		LOG.debug("┌───────────────────────────────────────────┐");
		LOG.debug("│ doSave()                                  │inVO:" + inVO);
		LOG.debug("└───────────────────────────────────────────┘");

		int flag = subjectService.doSaveSubject(inVO);
		String message = "";

		if (1 == flag) {
			message = inVO.getDetName() + "가 등록 되었습니다.";
		} else {
			message = inVO.getDetName() + "등록 실패.";
		}
