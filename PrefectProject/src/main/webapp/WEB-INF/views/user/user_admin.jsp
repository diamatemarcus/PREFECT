...
<script> // 세션에 연결된 이메일이 같을때 수정 가능하게 할 수 있는 자바 스크립트 기능 추가중
    ...
    document.addEventListener("DOMContentLoaded", function() {
        var emailInput = document.getElementById("email"); // 이메일 입력란 가져오기
        var passwordInput = document.getElementById("password");
        var telInput = document.getElementById("tel");
        var userEmail = "${outVO != null ? outVO.email : userSession.email}"; // 세션에 저장된 이메일 가져오기

        // 세션에 저장된 이메일과 입력란에 작성된 이메일이 같은 경우
        if (emailInput.value === userEmail) {
            emailInput.removeAttribute("readonly"); // 이메일 readonly 속성 제거하여 편집 가능하도록 설정
        }
