...
<style>
.readonly-div {
	cursor: default; /* 마우스 커서를 기본 값으로 변경하여 마우스 이벤트를 막음 */
}
</style>
...
<body>
  ...
  <jsp:include page="/WEB-INF/cmn/nav.jsp"></jsp:include> --%>
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"
					style="margin-bottom: 10px; color: #e44646; font-family: Montserrat; font-style: italic;">My
					Page</h1>
			</div>
					...
					<form action="#" name="userRegFrm">
						...
						<div class="mb-6">
							<label for="password" class="form-label">비밀번호</label> <input
								...
								id="password" placeholder="비밀번호를 입력 하세요."
								value="${outVO.password }" size="20" maxlength="30">
						</div>
						<div class="mb-6">
							<label for="tel" class="form-label">전화번호</label> <input
								type="text" class="form-control" name="tel" id="tel"
								placeholder="전화번호 수정" value="${outVO.tel }" size="20"
								maxlength="11">
						</div>
						<div type="hidden" class="mb-3" style="display: none;">
						...
						<div class="mb-3 mt-3">
							...
							<div class="readonly-div col-auto mb-auto" id="readonlyDiv">
						...
						<!-- 셀렉트 박스 -->
						<div class="mb-auto" style="display: none;">
						...
						<table id="licensesList" style="display: none;">
							<thead>
								<tr>
									<th>자격증명</th>
									<th>등록일</th>
	...
	<script>
		// 읽기 전용으로 만들 div 요소를 가져옴
		var readonlyDiv = document.getElementById("readonlyDiv");

		// 마우스 클릭, 키보드 이벤트 등을 막아 읽기 전용으로 만듦
		readonlyDiv.addEventListener("mousedown", function(event) {
			event.preventDefault(); // 기본 마우스 클릭 동작 막음
			event.stopPropagation(); // 상위 요소로의 이벤트 전파 막음
		});

		readonlyDiv.addEventListener("keydown", function(event) {
			event.preventDefault(); // 기본 키보드 이벤트 막음
			event.stopPropagation(); // 상위 요소로의 이벤트 전파 막음
		});
	</script>
