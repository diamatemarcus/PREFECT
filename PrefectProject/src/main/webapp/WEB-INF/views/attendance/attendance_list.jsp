...
<script>
	...
	function doSave() {
		...
		var calID = selectedDate.getFullYear() * 10000
				+ (selectedDate.getMonth() + 1) * 100 + selectedDate.getDate();

		console.log(calID);

		var saveButton = document.getElementById('doSave');
		var selectElements = document
				.querySelectorAll('#attendanceTable select#attendStatus');
		var traineeEmails = document
				.querySelectorAll('#attendanceTable input[id="traineeEmail"]');
...
<body>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-lg-12">
				<h2 class="page-header" style="text-align: center;">출석 체크</h2>
			</div>
		</div>
		<br> <br>
				...
				<tbody>
					<tr>
						...
						<td><input type="text" class="form-control ppl_input"
							...
							value="${course.courseName}_${course.numberOfTimes}회차"
							style="width: 110%;" maxlength="30"></td>
					</tr>
					<tr>
						<td class="form-label">훈련 기관명</td>
						<td><input type="text" class="form-control"
							name="academyName" id="academyName" size="20"
							value="${course.academyName}" style="width: 110%;" maxlength="21"
							readonly></td>
					</tr>
					<tr>
									...
									<div class="col-auto">
										<select id="attendStatus" name="attendStatus">
											...
											<c:forEach items="${attendStatusList}" var="status">
												...
												<%-- 일치하는 데이터를 찾았는지 여부를 저장하는 변수 --%>
												<c:forEach var="attendance" items="${attendances}">
													<c:if test="${attendance.trainee eq vo.email}">
														<option value="${status.detCode}"
															<c:if test="${attendance.attendStatus eq status.detCode}">selected</c:if>>${status.detName}</option>
								...
								<c:choose>
									<c:when test="${not empty attendances }">
										<td class="text-center"><input type="button" value="수정"
											class="button" id="doUpdate" onclick="doUpdate(this);">
										</td>
									</c:when>
								</c:choose>
							</tr>
