...
<body>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-lg-12">
				<h2 class="page-header" style="text-align: center;">성적</h2>
			</div>
		</div>
		<br> <br>
		<form action="#" method="get" name="subjectFrm"
			...
			<div class="row g-1 justify-content-end ">
				<div class="col-auto">
					<select name="searchDiv" id="searchDiv"
						class="form-select pcwk_select" onchange="doRetrieve(1);">
						...
						<c:when test="${not empty list}">
							<c:forEach var="vo" items="${list}">
								<tr>
									...
									<%-- 과목 이름을 출력 --%>
									<td class="text-center" data-code="${vo.subjectCode}"><c:forEach
											var="subject" items="${subjectCode}">
	...
	<script type="text/javascript">
		...
		document.addEventListener("DOMContentLoaded", function() {
			...
			document.getElementById('searchDiv').addEventListener(
					'change',
					function() {
						var selectedSubjectCode = this.value; // 선택된 과목 코드를 가져옵니다.
						var tableRows = document
								.querySelectorAll("#subjectTable tbody tr"); // 테이블의 모든 행을 선택합니다.

						// 각 행에 대해 반복하여 선택된 과목과 일치하는지 여부에 따라 표시합니다.
						tableRows.forEach(function(row) {
							var subjectCode = row.querySelector(
									"td:nth-child(2)")
									.getAttribute('data-code'); // 각 행의 과목 코드를 가져옵니다.
